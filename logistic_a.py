...
if (float(param[0]) == 1.0):
	...
	while(itr <= max_itr):
		...
		derivative = np.dot( X_f.transpose( ) , (function_H(X_f, W) - Y) )
...
####################################################################
	...
	while(itr <= max_itr):
		...
		derivative = np.dot( X_f.transpose() , (function_H(X_f, W) - Y) )
		function = function_L(W, X_f, Y)

		while( function_L( W - np.multiply(rate, derivative), X_f, Y) > function - rate*alpha*(np.linalg.norm(derivative,2))):
			rate = rate * beta
